...
const generateImage = async (imagePath) => {
    let fact = randomFact();
...
const getRandomImage = async (animal) => {
    try{
        ...
        console.log(">>>> IMAGE FROM PEXELS : ", image, image.src.medium)

        return image;

    } catch (e) {
        console.error("error get random image: ", e);
        getRandomImage(animal)
    }
}

const editImage = async (image, imagePath, fact) => {
    try{

        let imgURL = image.src.medium;
        let animalImage = await Jimp.read(imgURL).catch((error) => console.log("error read email jimp: ", error));
        ...
        console.error("error edit image: ", e);
    }
}
